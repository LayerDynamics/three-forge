...
import {
  Scene,
  Object3D,
  BufferGeometry, // Added BufferGeometry
  Material,
  Mesh,
} from "three";
import {
...
import { MemorySystemInstance } from "../MemorySystem/MemorySystem"; // Assuming this import exists
...
export class SceneGraphSystemClass {
  ...
  public removeObject(id: string): void {
    ...
    if (node) {
      // If the node has a parent, remove it from the parent's childrenIds
      if (node.parentId) {
        const parentNode = useSceneGraphStore.getState().getNode(node.parentId);
        if (parentNode) {
          useSceneGraphStore.getState().updateNode(parentNode.id, {
   ...
   * @param newParentId The ID of the new parent node. Can be undefined to detach.
   */
  public moveObject(childId: string, newParentId?: string): void {
    ...
    const newParentNode = newParentId
      ? useSceneGraphStore.getState().getNode(newParentId)
      : undefined;

    const childObject3D = this.object3DMap[childId];
    const newParentObject3D = newParentId ? this.object3DMap[newParentId] : undefined;

    if (childNode && childObject3D) {
      ...
      // Add to new parent in Three.js or to root scene
      if (newParentObject3D) {
        newParentObject3D.add(childObject3D);
      } else {
        this.threeScene.add(childObject3D);
      }

      if (this.config.debug) {
        console.log(
          `SceneGraphSystem: Moved object ${childId} to parent ${newParentId || 'root'}`
        );
      }
    } else if (this.config.debug) {
      console.warn(
