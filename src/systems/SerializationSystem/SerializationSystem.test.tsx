...
import { SerializedData, SerializedSceneGraph, SerializedBody, SerializedPlayerState } from '../../types/serialization.types';
...
describe('SerializationSystem', () => {
  ...
  it('should save game data correctly', async () => {
    const mockData: Partial<SerializedData> = {
      systems: {
        scene: { objects: {}, nodes: [] } as SerializedSceneGraph,
        physics: { bodies: [] as SerializedBody[] }
      },
      state: {
        player: { health: 100, position: [0, 0, 0], stamina: 100, experience: 0, level: 1, rotation: [0, 0, 0, 0] } as SerializedPlayerState
      }
    };
  ...
  it('should create and restore backups', async () => {
    const mockSaveData: Partial<SerializedData> = {
      state: { player: { health: 100, stamina: 100, experience: 0, level: 1, position: [0, 0, 0], rotation: [0, 0, 0, 0] } as SerializedPlayerState }
    };

    await serializationSystem.saveGame('slot1', mockSaveData);
    await serializationSystem['createBackup'](); // Accessing private method for testing
