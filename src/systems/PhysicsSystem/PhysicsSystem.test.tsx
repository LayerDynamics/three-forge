...
import { PhysicsSystemManager, handleRapierCollision, handleCannonCollision } from "./PhysicsSystem";
...
describe("PhysicsSystemManager", () => {
  ...
  it("should handle collision callbacks for RapierPhysics", () => {
    const onRapierCollideMock = jest.fn();
    const body: PhysicsBody = {
      id: "cylinder1",
      type: "dynamic",
      shape: "cylinder",
      ...
      args: [1, 2, 3, 32],
      mass: 1,
      onCollide: onRapierCollideMock,
    };

    physicsSystem.addBody(body);

    // Simulate a collision by invoking handleRapierCollision directly with correct event structure
    const otherBodyId = "box2";
    const contactPoint = new Vector3(0, 1, 0);

    const rapierEvent = {
      otherRigidBodyObject: () => ({ userData: { id: otherBodyId } }),
      contacts: [{ x: contactPoint.x, y: contactPoint.y, z: contactPoint.z }],
    };

    handleRapierCollision(rapierEvent, body, "rapier");

    expect(onRapierCollideMock).toHaveBeenCalledWith(otherBodyId, contactPoint);
    expect(onRapierCollideMock).toHaveBeenCalledTimes(1);
  });

  it("should handle collision callbacks for CannonPhysics", () => {
    const onCannonCollideMock = jest.fn();
    const body: PhysicsBody = {
      id: "sphere2",
      type: "dynamic",
      shape: "sphere",
      ...
      args: [0.5],
      mass: 1,
      onCollide: onCannonCollideMock,
    };

    physicsSystem.addBody(body);

    // Simulate a collision by invoking handleCannonCollision directly with correct event structure
    const otherBodyId = "box3";
    const contactPoint = new Vector3(1, 1, 1);

    const cannonEvent = {
      body: { userData: { id: otherBodyId } },
      contact: {
    ...
    handleCannonCollision(cannonEvent, body, "cannon");

    expect(onCannonCollideMock).toHaveBeenCalledWith(otherBodyId, contactPoint);
    expect(onCannonCollideMock).toHaveBeenCalledTimes(1);
  });
});
