...
export const useSceneGraphStore=create<SceneGraphState>()(
	devtools((set,get) => ({
			...
			set((state) => {
				...
				if(removedNode.parentId) {
					...
					if(parent) {
						...
						console.warn(
							`SceneGraph: Parent node ${removedNode.parentId} not found for node ${id}.`
						);
					}
				}
		...
		updateNode: (
			id: string,
			updatedFields: Partial<SceneGraphStoreNode>
		) => {
			set((state) => {
			...
			set((state) => {
				...
				if(currentParentId) {
					...
					if(currentParent) {
						...
						console.warn(
							`SceneGraph: Current parent node ${currentParentId} not found for child ${childId}.`
						);
					}
				}
