...
const PhysicsBodyButton: React.FC = () => {
  ...
  const handleAddBox = () => {
    ...
    const position = new Vector3(Math.random() * 5 - 2.5, 5, Math.random() * 5 - 2.5);
  ...
  const handleRemoveLastBox = () => {
    // Implement logic to remove the last added box
    // For simplicity, remove a random box
    const state = usePhysicsStore.getState();
    const boxIds = Object.keys(state.bodies).filter((id) => id.startsWith("box-"));
    if (boxIds.length > 0) {
  ...
  const handleRemoveLastSphere = () => {
    // Implement logic to remove the last added sphere
    // For simplicity, remove a random sphere
    const state = usePhysicsStore.getState();
    const sphereIds = Object.keys(state.bodies).filter((id) => id.startsWith("sphere-"));
