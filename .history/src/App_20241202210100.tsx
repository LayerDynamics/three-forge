...
import { AbilitySystemClass } from "./systems/AbilitySystem/AbilitySystem";
import { WeaponSystemClass } from "./systems/WeaponSystem/WeaponSystem";
import { PhysicsSystemClass } from "./systems/PhysicsSystem/PhysicsSystem";
import { AnimationSystemClass } from "./systems/AnimationSystem/AnimationSystem";
import { AssetLoadingSystemClass } from "./systems/AssetLoadingSystem/AssetLoadingSystem";
import { LogicEngineClass } from "./systems/LogicEngine/LogicEngine";
...
import { LogicComponent, GameEvent, Asset } from "./types/logic.types"; // Ensure Asset is correctly exported from this path
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Instantiate Systems
    const abilitySystem = AbilitySystemClass.getInstance();
    const weaponSystem = WeaponSystemClass.getInstance();
    const physicsSystem = PhysicsSystemClass.getInstance({
      engine: "cannon",
      gravity: new Vector3(0, -9.81, 0),
      debug: true
    });
    const animationSystem = AnimationSystemClass.getInstance();
    const assetLoadingSystem = AssetLoadingSystemClass.getInstance();
    const logicEngine = LogicEngineClass.getInstance();
    const sceneGraphSystem = SceneGraphSystemClass.getInstance({ debug: true }, sceneGraphRef.current as Scene);

    // Cleanup Systems on Unmount
    return () => {
      animationSystem.reset();
      assetLoadingSystem.reset();
      logicEngine.reset();
      sceneGraphSystem.reset();
      // Add additional cleanup for other systems if necessary
    };
  }, []);
