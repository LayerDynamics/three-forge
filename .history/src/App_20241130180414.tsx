...
import { Vector3, Mesh, AnimationClip, AnimationMixer, NumberKeyframeTrack, LoopRepeat } from "three"; // Imported Vector3 directly
...
import AnimationController from "./components/UI/AnimationController"; // Ensure this component exists
import "./App.css";

const AnimatedBox: React.FC = () => {
  const meshRef = useRef<Mesh>(null); // Corrected ref type to Mesh
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    addWeapon({
      ...
      fire: (target?: Vector3) => { // Changed from THREE.Vector3 to Vector3
    ...
    <div className="App">
      <Canvas>
        <PhysicsSystem config={{ engine: "cannon", gravity: new Vector3(0, -9.81, 0), debug: true }}>
          ...
          <PhysicsBodyButton />
          <AnimationController /> {/* Ensure this component exists */}
        </PhysicsSystem>
      </Canvas>
    </div>
  );
