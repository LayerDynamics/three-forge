...
export class LevelManager extends React.Component {
  ...
  try {
    ...
    // Set up global lighting parameters
    await lightingSystem.setGlobalParameters({
      exposure: lightingData.exposure || 1.0,
      gamma: lightingData.gamma || 2.2,
      ambient: lightingData.ambient || { intensity: 0.2, color: 0xffffff }
    });

    // Place all lights
    for (const light of lightingData.lights) {
      const lightObject = await lightingSystem.createLight({
        type: light.type,
    ...
    // Set up environment
    if (lightingData.environment) {
      await lightingSystem.setEnvironment({
        type: lightingData.environment.type,
        texture: lightingData.environment.texture,
        intensity: lightingData.environment.intensity
      });
    }

    // Initialize light probes if needed
    if (lightingData.probes) {
      await lightingSystem.setupLightProbes(lightingData.probes);
    }

    // Apply post-processing effects
    if (lightingData.postProcessing) {
      this.postProcessingSystem.updateConfig(lightingData.postProcessing);
    }
