...
type Module={
	...
	// Define other necessary functions exposed by the encoder
};

export class TextureCompressor {
	private static instance: TextureCompressor|null=null;

	private basisModule: Module|null=null;
	private ktx2Module: Module|null=null;
	...
	private async loadBasisModule(): Promise<void> {
		...
		return new Promise((resolve,reject) => {
			...
			script.src='/encoders/basis_transcoder/basis_transcoder.js';
			script.onload=() => {
				if((window as any).BasisTranscoder) {
					// Initialize the module
					(window as any).BasisTranscoder().then((module: Module) => {
						this.basisModule=module;
						resolve();
					}).catch(reject);
				} else {
					reject(new Error('BasisTranscoder module not found on window'));
				}
			};
			script.onerror=() => reject(new Error('Failed to load BasisTranscoder script'));
	...
	private async loadKTX2Module(): Promise<void> {
		...
		return new Promise((resolve,reject) => {
			const script=document.createElement('script');
			script.src='/encoders/ktx2_encoder/libktx.js';
			script.onload=() => {
				if((window as any).Libktx) {
					// Initialize the module
					(window as any).Libktx().then((module: Module) => {
						this.ktx2Module=module;
						resolve();
					}).catch(reject);
				} else {
					reject(new Error('Libktx module not found on window'));
				}
			};
			script.onerror=() => reject(new Error('Failed to load Libktx script'));
			document.body.appendChild(script);
		});
	}
