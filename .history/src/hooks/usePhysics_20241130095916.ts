...
import {PhysicsBody,ColliderShape,PhysicsConfig} from "../types/physics.types";
...
export const usePhysics=() => {
	...
	const addBox=useCallback(
		(id: string,position: Vector3,size: Vector3,type: "dynamic"|"static"="dynamic") => {
			createBody({
				id,
				type,
				shape: "box",
				...
				onCollide: (otherId,contactPoint) => {
					console.log(`Box ${id} collided with ${otherId} at ${contactPoint.toArray()}`);
				},
			});
		},
		[createBody]
	...
	const addSphere=useCallback(
		(id: string,position: Vector3,radius: number,type: "dynamic"|"static"="dynamic") => {
			createBody({
				id,
				type,
				shape: "sphere",
				...
				mass: type==="dynamic"? 1:0,
				onCollide: (otherId,contactPoint) => {
					console.log(`Sphere ${id} collided with ${otherId} at ${contactPoint.toArray()}`);
				},
			});
		},
		[createBody]
