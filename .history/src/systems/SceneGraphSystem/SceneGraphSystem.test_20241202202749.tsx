...
import { SceneGraphSystemClass } from "./SceneGraphSystem"; // Changed to named import
import { useSceneGraphStore } from "../../stores/sceneGraphStore";
import { useLogicStore } from "../../stores/logicStore"; // Imported useLogicStore
...
describe("SceneGraphSystem", () => {
  ...
  beforeEach(() => {
    ...
    useLogicStore.setState({
      ...
      enqueueEvent: jest.fn((event: GameEvent) => {
        ...
        logicComponents[event.type] = {
          id: `component_${event.type}`,
          execute: jest.fn(),
        };
      }),
  ...
  it("should handle game state transitions based on events", () => {
    ...
    // Enqueue events
    (useLogicStore.getState().enqueueEvent as jest.Mock).mockImplementation((event: GameEvent) => {
      useLogicStore.setState({
