...
const createMockState = (): MockState => {
  ...
  const mockState = state as MockState;

  mockState.addNode = jest.fn((node: SceneGraphStoreNode) => {
    mockState.nodes[node.id] = { ...node };
  });

  mockState.removeNode = jest.fn((id: string) => {
    delete mockState.nodes[id];
  });

  mockState.setParent = jest.fn((childId: string, parentId?: string) => {
  ...
  mockState.getNode = jest.fn((id: string) => mockState.nodes[id]);

  mockState.updateNode = jest.fn((id: string, updates: Partial<SceneGraphStoreNode>) => {
