...
// Arrays to capture addNode calls
const addNodeCalls: SceneGraphStoreNode[] = [];
...
describe("SceneGraphSystem", () => {
  ...
  beforeEach(() => {
    ...
    jest.resetAllMocks();
    addNodeCalls.length = 0; // Clear previous calls
    ...
    // Modify mockAddNode to capture a copy of each node when called
    mockAddNode = jest.fn((node: SceneGraphStoreNode) => {
      addNodeCalls.push({ ...node }); // Capture a shallow copy at call time
    ...
    // Mock the useSceneGraphStore.getState and its methods
    ...
    // Mock the useSceneGraphStore.setState if needed
    ...
    // Mock the useLogicStore.getState and its methods
    ...
    // Mock the useLogicStore.setState if needed
  ...
  it("should handle adding multiple objects and resetting correctly", () => {
    ...
    // Assert: addNode was called twice with storeNode[0] and storeNode[1]
