...
import { SceneGraphConfig, SceneGraphNode, SceneGraphStoreNode } from "../../types/sceneGraph.types";
...
export class SceneGraphSystemClass {
  ...
  public addObject(id: string, name: string, object3D: Object3D, parentId?: string): void {
    ...
    const node: SceneGraphStoreNode = { id, name, childrenIds: [], parentId };
    ...
    if (parentId) {
      ...
      if (parentNode) {
        ...
        if (parentObject3D) {
          ...
          console.warn(`SceneGraphSystem: Parent Object3D for ${parentId} not found.`);
          this.threeScene.add(object3D);
        }
      } else {
        console.warn(`SceneGraphSystem: Parent node ${parentId} not found for object ${id}. Adding to root scene.`);
        this.threeScene.add(object3D);
      }
    } else {
      // Add to root scene
  ...
  public moveObject(childId: string, newParentId: string): void {
    ...
    if (childNode && newParentNode && childObject3D && newParentObject3D) {
