...
describe('CameraSystem', () => {
  ...
  it('should update target and offset', () => {
    ...
    expect(activeController!.getOffset()).toEqual(newOffset);
    expect(activeController!.getShakeState().active).toBe(false);

    // Simulate updates to allow position to lerp towards target + offset
    ...
    const expectedPosition = newTarget.clone().add(newOffset); // (1, 6, 6)
    const actualPosition = activeController!.getPosition();

    // Since lerping with factor 0.1 per update for 50 updates,
    // the position should be very close to the desired position
    expect(actualPosition.x).toBeCloseTo(expectedPosition.x, 1);
    expect(actualPosition.y).toBeCloseTo(expectedPosition.y, 1);
    expect(actualPosition.z).toBeCloseTo(expectedPosition.z, 1);
  });
