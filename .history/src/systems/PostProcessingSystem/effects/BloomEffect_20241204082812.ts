...
/**
 * Enum for different blend functions.
 */
enum BlendFunction {
	ADD='ADD',
	MULTIPLY='MULTIPLY',
...
/**
 * Base Effect class for post-processing effects.
 */
class Effect {
	name: string;
	shader: string;
	...
	constructor(name: string,shader: string,options: {blendFunction: BlendFunction; uniforms: Map<string,Uniform<any>>}) {
		this.name=name;
		this.shader=shader;
		this.blendFunction=options.blendFunction;
		this.uniforms=options.uniforms;
	}

	/**
	 * Dispose of the effect's resources.
	 */
	dispose(): void {
