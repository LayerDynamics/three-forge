...
function blend(base: Vector3, blend: Vector3, blendMode: string): Vector3 {
	switch (blendMode) {
		case 'ADD':
			return base.add(blend);
		case 'MULTIPLY':
			return base.multiply(blend);
		case 'SCREEN':
			return new Vector3(
				1 - (1 - base.x) * (1 - blend.x),
				1 - (1 - base.y) * (1 - blend.y),
				1 - (1 - base.z) * (1 - blend.z)
			);
		default:
			return base;
	}
...
function applyBloomEffect(
	...
	const renderTarget = new WebGLRenderTarget(inputBuffer.width, inputBuffer.height, {
	...
	// Apply bloom effect
	const bloomShaderMaterial = new ShaderMaterial({
		uniforms: {
	...
	const quad = new Mesh(new PlaneGeometry(2, 2), bloomShaderMaterial);
