...
import {AssetLoadingSystemClass} from "./AssetLoadingSystem";
...
describe("AssetLoadingSystem",() => {
	const assetLoadingSystem=new AssetLoadingSystemClass({debug: true});
	...
	it("should handle model asset loading error",(done) => {
		const mockLoad=jest.fn((url,onLoad,onProgress,onError) => {
			// Simulate load error
		...
		const handleAssetError=(data: any) => {
			...
			const loadedAsset=useAssetStore.getState().assets["model2"];
			expect(loadedAsset).toBeDefined();
			expect(loadedAsset.loaded).toBe(false);
			expect(loadedAsset.error).toBe("Failed to load model");
			done();
		};
	...
	it("should load a sound asset successfully",(done) => {
		...
		const handleAssetLoaded=(data: any) => {
			...
			expect(loadedAsset).toBeDefined();
			expect(loadedAsset.loaded).toBe(true);
			expect(loadedAsset.data).toEqual({uuid: "sound1"});
			done();
		};
	...
	it("should handle unsupported asset types",(done) => {
		...
		const handleAssetError=(data: any) => {
			...
			expect(data.error).toBe("Unsupported asset type");
			const loadedAsset=useAssetStore.getState().assets["unknown1"];
			expect(loadedAsset).toBeDefined();
			expect(loadedAsset.loaded).toBe(false);
			expect(loadedAsset.error).toBe("Unsupported asset type");
			done();
		};
