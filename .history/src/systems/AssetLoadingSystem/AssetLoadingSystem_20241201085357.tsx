...
import {
  TextureLoader,
  AudioLoader,
  // Removed VideoLoader as it's not part of three.js by default
  VideoTexture,
  AudioListener,
  Audio,
} from "three";
...
import { AudioManager } from "../../utils/AudioManager"; // Ensure this class is correctly implemented

// Define a generic loader type with appropriate generics
interface Loader<T> {
  load(
    url: string,
    onLoad: (data: T) => void,
...
export class AssetLoadingSystemClass {
  ...
  private constructor(config: AssetConfig = { debug: false }) {
    ...
    this.loaders = {
      ...
      // video: null, // Removed VideoLoader
  ...
  public loadAsset(asset: Asset): void {
    ...
    switch (asset.type) {
      ...
      // Removed "video" case as VideoLoader is not supported by default
