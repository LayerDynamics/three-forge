...
export class LODSystem {
  ...
  /**
   * Resets the singleton instance of LODSystem.
  ...
  public update(): void {
    ...
    Object.values(objects).forEach((lodObject: LODObject) => {
      ...
      if (newLevel !== lodObject.currentLevel) {
        ...
        if (Array.isArray(lodObject.object.material)) {
          lodObject.object.material.forEach((mat: Material) => mat.dispose());
   ...
   * @param distanceOverride Optional distance override. If not provided, calculated from camera.
  ...
  private getLODLevelIndex(lodObject: LODObject, distanceOverride?: number): number {
    ...
    for (let i = 0; i < lodObject.levels.length; i++) {
      if (distance <= lodObject.levels[i].distance) { // Changed from < to <=
        return i;
      }
    }
