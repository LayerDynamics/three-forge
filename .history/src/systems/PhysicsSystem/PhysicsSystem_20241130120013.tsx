...
// Import necessary hooks from @react-three/cannon with alias for Physics
import {
...
// Import RigidBody from @react-three/rapier with alias for Physics
import {
...
const CapsuleBodyCannon: React.FC<{ body: PhysicsBody }> = ({ body }) => {
  const [ref] = useCylinder<Mesh>(() => ({
    ...
    position: [body.position.x, body.position.y, body.position.z],
    rotation: [body.rotation.x, body.rotation.y, body.rotation.z],
    args: body.args as [number, number] || [0.5, 2], // radiusTop, radiusBottom
    onCollide: (e: any) => {
      handleCollision(e, body);
    },
  }));
...
const CylinderBodyCannon: React.FC<{ body: PhysicsBody }> = ({ body }) => {
  const [ref] = useCylinder<Mesh>(() => ({
    ...
    position: [body.position.x, body.position.y, body.position.z],
    rotation: [body.rotation.x, body.rotation.y, body.rotation.z],
    args: body.args as [number, number, number, number] || [1, 1, 2, 32], // radiusTop, radiusBottom, height, numSegments
    onCollide: (e: any) => {
      handleCollision(e, body);
    },
  }));
...
const ConeBodyCannon: React.FC<{ body: PhysicsBody }> = ({ body }) => {
  ...
  const [ref] = useCylinder<Mesh>(() => ({
    ...
    position: [body.position.x, body.position.y, body.position.z],
    rotation: [body.rotation.x, body.rotation.y, body.rotation.z],
    args: body.args as [number, number, number] || [0.5, 0, 2], // radiusTop, radiusBottom, height
    onCollide: (e: any) => {
      handleCollision(e, body);
    },
  }));
