...
import { RigidBody, Physics as RapierPhysics, RapierCollisionEvent } from '@react-three/rapier';
import { Physics as CannonPhysics, useBox, useSphere, useCylinder, usePlane, CollisionEvent } from '@react-three/cannon';
...
/**
 * handleCollision function processes collision events from both Rapier and Cannon engines.
 * It extracts the relevant information and triggers the onCollide callback if defined.
 */
export const handleCollision = (
  event: RapierCollisionEvent | CollisionEvent,
  body: PhysicsBody,
  engine: PhysicsEngine
) => {
  if (engine === "rapier" && isRapierCollisionEvent(event)) {
    const rapierEvent = event;
    const otherRigidBody = rapierEvent.otherRigidBodyObject?.();
    const otherBodyId = otherRigidBody?.userData?.id;

    const contactPoints = rapierEvent.contacts;
  ...
  } else if (engine === "cannon" && isCannonCollisionEvent(event)) {
    const cannonEvent = event;
    const otherBody = cannonEvent.body;
    const otherBodyId = otherBody?.userData?.id;

    const contactPoint = cannonEvent.contactPoint;
...
/**
 * Type guard to check if the event is a RapierCollisionEvent.
 */
const isRapierCollisionEvent = (event: RapierCollisionEvent | CollisionEvent): event is RapierCollisionEvent => {
  return (event as RapierCollisionEvent).otherRigidBodyObject !== undefined;
};

/**
 * Type guard to check if the event is a CollisionEvent from Cannon.
 */
const isCannonCollisionEvent = (event: RapierCollisionEvent | CollisionEvent): event is CollisionEvent => {
  return (event as CollisionEvent).body !== undefined && (event as CollisionEvent).contactPoint !== undefined;
};
      ...
      {Object.values(bodies).map((body) => {
        if (engine === "cannon") {
          ...
          switch (body.shape) {
            ...
            case "capsule":
              // Use compound collider if supported
            ...
            case "plane":
              // Use 'cuboid' to simulate a plane
...
const CapsuleBodyCannon: React.FC<{ body: PhysicsBody; engine: PhysicsEngine }> = ({ body, engine }) => {
  ...
  return (
    <mesh ref={ref}>
      <cylinderGeometry args={(body.args as [number, number]) || [0.5, 2, 32]} />
...
const BoxBodyRapier: React.FC<{ body: PhysicsBody; engine: PhysicsEngine }> = ({ body, engine }) => {
  return (
    <RigidBody
      ...
      onCollisionEnter={(event: RapierCollisionEvent) => {
