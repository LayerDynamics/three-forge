...
// Exported handleCollision function for testing purposes
export const handleCollision = (
  event: any, // Using 'any' since 'CollisionEvent' is not exported
  body: PhysicsBody,
  engine: PhysicsEngine
) => {
  if (engine === "rapier") {
...
const BoxBodyCannon: React.FC<{ body: PhysicsBody; engine: PhysicsEngine }> = ({ body, engine }) => {
  ...
  return <mesh ref={ref as React.RefObject<any>} />;
};
...
export const PhysicsSystem: React.FC<PhysicsSystemProps> = ({ config, children }) => {
  ...
  return (
    <>
      ...
      {engine === "rapier" && (
        <Physics gravity={[gravity.x, gravity.y, gravity.z]} debug={debug}>
...
const RapierPhysicsDebugComponent: React.FC = () => {
  // If Rapier provides a Debug component, import and use it
