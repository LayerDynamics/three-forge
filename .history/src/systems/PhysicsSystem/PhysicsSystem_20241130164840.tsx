...
import { Physics as CannonPhysics, useBox, useSphere, useCylinder, usePlane } from '@react-three/cannon';
...
// Define local interfaces for collision events

/**
 * Interface representing a collision event from Rapier.
 */
interface RapierCollisionEvent {
  otherRigidBodyObject: () => { userData: { id: string } } | undefined;
  contacts: Array<{ x: number; y: number; z: number }>;
}

/**
 * Interface representing a collision event from Cannon.
 */
...
export const handleCollision = (
  event: RapierCollisionEvent | CannonCollisionEvent,
  body: PhysicsBody,
  engine: PhysicsEngine
) => {
  if (engine === "rapier" && 'otherRigidBodyObject' in event) {
    const rapierEvent = event as RapierCollisionEvent;
  ...
  } else if (engine === "cannon" && 'body' in event && 'contactPoint' in event) {
    const cannonEvent = event as CannonCollisionEvent;
