...
export class PhysicsSystemManager {
  ...
  public initialize() {
    ...
    if (this.refCount === 1) { // Only initialize once
  ...
  public cleanup() {
    if (this.refCount > 0) {
      ...
      if (this.refCount === 0 && this.isInitialized) { // Only clean up when no more references
        if (this.config.engine === 'rapier') {
      ...
      console.warn('PhysicsSystemManager has no active references to clean up.');
    }
  }
